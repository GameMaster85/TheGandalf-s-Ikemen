...
==============================| Gouki by Phantom.of.the.Server Edited by mwryly |==========================
                               ¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯           
...
This character can choose between two normal movesets and zankuu hadoken, SF4 moveset and CVS moveset. In this character's folder, there is a file called "Config.txt". Choose which moveset you want to be able to perform by editing it like in the following example: 
...
This means that Gouki will perform the SF4 normal moveset. To make him perform CVS normal moveset instead, simply change the number to the right of "var(42) = " like in the following example: 

[State 7800, GOUKI NORMALS: 0 = SFIV, 1 = CVS default]
type = ParentVarSet
trigger1 = 1
var(42) = 1 ;CHANGE THIS LINE

NOTICE
 -"var(42)" is for Normal Gouki ONLY while "var(43)" is for Shin Gouki
 -"var(44)" will determine whether the Oni normal moves can be or cannot be perfromed. 1 = on or 0 = off. (Shin Gouki mode only)
...
(N) - Normal Gouki mode only
(S) - Shin Gouki mode only
...
Only Normal Gouki can use them.

<NORMAL>

.Zugai Hasatsu:             F + y
.(CVS)Senpuu Kyaku:              F + b
...
.(S)Gankauchi:              B + y
.Kikoku Tsuki:               2p
.(S)Target Combo 1:         (close)y, z
.(S)Target Combo 2:         b + y, z

<SPECIAL>

.Gou Hadou Ken(EX):             D, DF, F, p
.Shakunetsu Hadou Ken(EX):      F, DF, D, DB, B, p
.Gou Shouryuu Ken(EX):          F, D, DF, p
.Zankuu Hadou Ken(EX):          D, DF, F, p  (Air)
.Tatsumaki Zankuu Kyaku(EX):    D, DB, B, k (Air also)
.Ashura Senkuu Zenpou:      F, D, DF, 3p/3k
.Ashura Senkuu Kouhou:      B, D, DB, 3p/3k
.(S)Zenpou Tenshin:         D, DB, B, p
.Hyakki Shuu(EX):               F, D, DF, k
   .Hyakki Gou Zan:            do nothing
   .Hyakki Gou Shou:           p
   .Hyakki Gou Jin:            k
   .(S)Hyakki Gou Dan:         U + k
   .Hyakki Gou Sai:            F/B + p  (near opponent)
   .Hyakki Gou Tsui:           F/B + k  (near opponent)
.(S)Tenma Shurettou:        D, D, p or k
...
.(N)Tenma Gou Zankuu Agyo:       D, DF, F, D, DF, F, 2p  (Air)
.(N)Messatsu Gou Hadou Agyo:     D, DB, B, D, DB, B, 2p
.(S)Tenma Shinzui Heki:          D, DF, F, D, DF, F, k  (Air)

<Lv3 SUPERS>
.(N)Kongou Kokuretsu Zan:   D, D, D, 2p
.(S)Misogi:                 D, D, D, 2k
...
- Ex Zankuu Hadoken can ba canceled out of Ex Hyakki Shuu, only on startup of the move;

- Ex Hyakki Gou Shou can ground bounce opponent 
...
- Kongou Kokuretsu Zan destroy projectiles;

- Kongou Kokuretsu Zan can be canceled out of a taunt;
...
Z or C -> Gouki stomps the ground, surrounded by an aura while saying "Orokana!" (You fool!).
...
<02.09.2012>
- See Editor's Readme for to see all changes 
